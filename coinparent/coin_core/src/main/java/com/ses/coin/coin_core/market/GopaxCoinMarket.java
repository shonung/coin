...
public class GopaxCoinMarket implements IMarket {
  private static final String TAG = BithumbCoinMarket.class.getSimpleName();

  private MarketInfo marketInfo = null;
  private GopaxCoinCheckerManager checkerManager = null;
  private IStatusCollector statusCollector = null;
  
  public GopaxCoinMarket(MarketInfo marketInfo) { 
    Logger.info(TAG, marketInfo.getMarketName() + " Coin Market is created. host url : " + marketInfo.getHostUrl());
    this.marketInfo = marketInfo;
    checkerManager = new GopaxCoinCheckerManager(marketInfo);
    statusCollector = StatusCollectorFactory.GOPAX_COIN.create();
